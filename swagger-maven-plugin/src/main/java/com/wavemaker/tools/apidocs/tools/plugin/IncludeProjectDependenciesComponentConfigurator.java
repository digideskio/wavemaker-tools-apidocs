...
public class IncludeProjectDependenciesComponentConfigurator extends AbstractComponentConfigurator {
    ...
    private void addProjectDependenciesToClassRealm(ExpressionEvaluator expressionEvaluator, ClassRealm containerRealm)
        ...
        runtimeClasspathElements.addAll(retrieveCustomJars(expressionEvaluator));
    ...
    /**
     * This function will read the configuration from the pom and returns the list of user specified jar folders
     *
     * @param expressionEvaluator
     * @return
     * @throws ComponentConfigurationException
     */
    private List<String> getJarFoldersFromConfig(ExpressionEvaluator expressionEvaluator)
            throws ComponentConfigurationException {
        ...
        try {
            ...
            throw new ComponentConfigurationException("There was a problem evaluating: ${mojo}", e);
        }
        Xpp3Dom jarFolders = mojoExec.getConfiguration().getChild(JAR_CONFIG_ENTRY);
    ...
    private File getProjectBuildRoot(ExpressionEvaluator expressionEvaluator) throws ComponentConfigurationException {
        String finalName = "";
        try {
            finalName = (String) expressionEvaluator.evaluate("${project.build.finalName}");

        } catch (ExpressionEvaluationException e) {
            throw new ComponentConfigurationException("There was a problem evaluating: ${project.build.finalName}", e);
        }
        File baseDir = null;
        try {
            baseDir = new File((String) expressionEvaluator.evaluate("${project.build.directory}"));
        } catch (ExpressionEvaluationException e) {
            throw new ComponentConfigurationException("There was a problem evaluating: ${project.build.directory}", e);
        }
        return new File(baseDir, finalName);
    }

    private List<String> retrieveCustomJars(ExpressionEvaluator expressionEvaluator)
            throws ComponentConfigurationException {
        ...
        if (jarDirs.size() > 0) {
            ...
            try {
                ...
                throw new ComponentConfigurationException("There was a problem in setting classpath", e);
            }
        }
        return paths;
    }
